#!/usr/bin/env python3
"""
hp_project_hp_project_restore.py — STANDALONE project restore script (no dependencies).

PURPOSE: Restore the entire Harry Potter 21+ Fanfic project after a server rollback.
         This script survives rollbacks because it is uploaded to GitHub.

WHAT IT RESTORES:
  - download/ (HTML files: Bible, Outline, Canon Reference, prompts)
  - scripts/ (Python scripts, agents, character data, image URLs)
  - restrictions/ (banned words, sentences, context limits)
  - upload/ (system prompt, uploaded HTML files)
  - project.json (project config)

DOES NOT RESTORE:
  - Actual image files (only URLs are backed up)
  - GitHub PAT (you need to create a new one)

STEPS AFTER ROLLBACK:
  1. Download this script from GitHub:
     curl -o /home/z/my-project/hp_project_hp_project_restore.py https://raw.githubusercontent.com/hungnguyenhoang0103/harry_potter-fanfic-z.ai-images-9ad58zx/staging/hp_project_hp_project_restore.py

  2. List available backups:
     python3 hp_project_hp_project_restore.py list-github

  3. Download + restore in one step:
     python3 hp_project_hp_project_restore.py restore-timestamp <timestamp>

  4. Create new GitHub PAT (if needed) and save:
     echo "github_pat_xxx" > scripts/.github_token

  5. Verify restored files:
     ls download/*.html scripts/agents/*.py
...
1. This file (hp_project_restore.py)
2. A backup ZIP file (local or downloaded from GitHub)

Usage:
  python3 hp_project_restore.py list                          List local backups
  python3 hp_project_restore.py list-github                   List GitHub backups (public, no PAT needed)
  python3 hp_project_restore.py download <timestamp>          Download backup from GitHub
  python3 hp_project_restore.py restore <zip_path>            Restore from local ZIP
  python3 hp_project_restore.py restore-timestamp <timestamp> Download + restore in one step

GitHub repo: hungnguyenhoang0103/harry_potter-fanfic-z.ai-images-9ad58zx
Branch: staging
Backup folder: backups/

This file should be saved to: /home/z/my-project/hp_project_restore.py (ROOT level, not in scripts/)
Also uploaded to GitHub: https://raw.githubusercontent.com/hungnguyenhoang0103/harry_potter-fanfic-z.ai-images-9ad58zx/staging/hp_project_restore.py
"""
import sys
import os
import zipfile
...
def main():
    ...
    if cmd == "list":
        ...
        if len(sys.argv) < 3:
            print("Usage: python3 hp_project_restore.py download <timestamp>")
            return
        download_backup(sys.argv[2])
    elif cmd == "restore":
        if len(sys.argv) < 3:
            print("Usage: python3 hp_project_restore.py restore <zip_path>")
            return
        restore_backup(sys.argv[2])
    elif cmd == "restore-timestamp":
        if len(sys.argv) < 3:
            print("Usage: python3 hp_project_restore.py restore-timestamp <timestamp>")
            return
